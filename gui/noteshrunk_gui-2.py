...
class PreviewWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:  # type: ignore[override]
        ...
        orig_rect = QtCore.QRectF(
            -self._original.width(),
            -self._original.height() / 2,
            self._original.width(),
            self._original.height(),
        )
        proc_rect = QtCore.QRectF(
            0,
            -self._processed.height() / 2,
        ...
        # Simple dividing line between halves
        split_x = 0
...
class SettingsPanel(QtWidgets.QScrollArea):
    ...
    def to_args(self) -> List[str]:
        ...
        # NOTE: due to a quirk in noteshrunk.py's argparse type for --threshold_empty,
        # passing the argument explicitly can cause a conversion error. We therefore
        # rely on its built-in default (2) and *do not* pass --threshold_empty here.
        # The spinbox is informative only unless the CLI is fixed.
