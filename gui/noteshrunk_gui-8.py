...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        ...
        # Constrain side panels: they have a preferred width but should not grow without bound
        left_policy = self.page_strip.sizePolicy()
        left_policy.setHorizontalPolicy(QtWidgets.QSizePolicy.Fixed)
        left_policy.setHorizontalStretch(0)
        self.page_strip.setSizePolicy(left_policy)
        self.page_strip.setMaximumWidth(220)

        right_policy = self.settings_panel.sizePolicy()
        right_policy.setHorizontalPolicy(QtWidgets.QSizePolicy.Fixed)
        right_policy.setHorizontalStretch(0)
        self.settings_panel.setSizePolicy(right_policy)
        self.settings_panel.setMaximumWidth(360)

        # Preview should take remaining space and be resizable
        center_policy = self.preview_widget.sizePolicy()
        center_policy.setHorizontalPolicy(QtWidgets.QSizePolicy.Expanding)
        center_policy.setHorizontalStretch(1)
        self.preview_widget.setSizePolicy(center_policy)
    ...
    def _build_central_layout(self) -> None:
        ...
        # Initial sizes; side panels have fixed max width, preview takes the rest
        splitter.setSizes([200, 800, 340])
