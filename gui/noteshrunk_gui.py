...
class PreviewWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:  # type: ignore[override]
        ...
        # Draw "Processed" / "Original" labels
        font = painter.font()
        font.setBold(True)
        ...
        proc_rect = fm.boundingRect(proc_text)
        orig_rect = fm.boundingRect(orig_text)

        # Processed (Bottom Left)
        bg_proc = QtCore.QRectF(10, self.height() - 10 - proc_rect.height() - 8, proc_rect.width() + 16, proc_rect.height() + 8)
        painter.setBrush(QtGui.QColor(0, 0, 0, 150))
        painter.setPen(QtCore.Qt.NoPen)
        painter.drawRoundedRect(bg_proc, 4, 4)
        painter.setPen(QtCore.Qt.white)
        painter.drawText(bg_proc, QtCore.Qt.AlignCenter, proc_text)

        # Original (Bottom Right)
        bg_orig = QtCore.QRectF(self.width() - 10 - orig_rect.width() - 16, self.height() - 10 - orig_rect.height() - 8, orig_rect.width() + 16, orig_rect.height() + 8)
        painter.setBrush(QtGui.QColor(0, 0, 0, 150))
        painter.setPen(QtCore.Qt.NoPen)
        painter.drawRoundedRect(bg_orig, 4, 4)
        painter.setPen(QtCore.Qt.white)
        painter.drawText(bg_orig, QtCore.Qt.AlignCenter, orig_text)
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        ...
        # Calculate screen coordinate of the split line
        center_x = self.width() / 2
        split_rel = -w / 2 + w * self._split_ratio
        screen_split_x = center_x + self._pan.x() + split_rel * self._zoom

        # Hit test in screen coordinates (generous 15px margin)
        if abs(event.position().x() - screen_split_x) < 15:
...
class SettingsPanel(QtWidgets.QScrollArea):
    ...
    def _slider_with_spin(self, label: str, spin: QtWidgets.QAbstractSpinBox, *, is_float: bool = False, scale: float = 1.0) -> QtWidgets.QWidget:
        ...
        slider = NoWheelSlider(QtCore.Qt.Horizontal, row)
