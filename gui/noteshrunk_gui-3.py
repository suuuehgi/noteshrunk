...
- Settings mapping 1:1 to noteshrunk CLI arguments (with a few safe
  workarounds for CLI quirks)
...
# ---------------------------------------------------------------------------
# Spin boxes that ignore wheel events (avoid accidental changes)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Palette editor (swatches + text toggle)
# ---------------------------------------------------------------------------


class PaletteEditor(QtWidgets.QWidget):
    """Widget that edits a comma-separated hex palette via swatches or raw text.

    The underlying value is always a string of comma-separated hex codes; swatch
    and text modes are just two views over the same data.
    """
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        # Default to swatch view; initialise with empty list
    ...
    def set_palette_text(self, text: str) -> None:
        ...
        # Keep line edit mirrored in case user switches modes
    ...
    def _on_toggle(self, checked: bool) -> None:
        if checked:
            ...
            # switch to swatch mode; parse text first
    ...
    def _on_text_changed(self, text: str) -> None:
        # Only update model when in text mode; swatch view will sync on toggle
...
class RenderWorker(QtCore.QThread):
    ...
    def run(self) -> None:  # type: ignore[override]
        ...
        if exit_code != 0:
            ...
            # WORKAROUND: noteshrunk.py has a bug where it crashes in the finally block
            # when outputting to non-PDF formats because `tmp_dir` is unbound.
            # The image is actually saved successfully before this crash happens.
            if "UnboundLocalError" in out and "tmp_dir" in out:
                self.log.emit("\n[GUI] Ignored UnboundLocalError from noteshrunk.py (known bug for non-PDF outputs).\n")
            else:
...
class PageThumbnailWidget(QtWidgets.QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.badge_label.setStyleSheet(
            "QLabel { background: #444; color: white; border-radius: 8px; padding: 2px 6px; }"
        )
...
class PreviewWidget(QtWidgets.QWidget):
    """Composite preview with draggable before/after split."""

    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        super().__init__(parent)
        self._original: Optional[QtGui.QPixmap] = None
        self._processed: Optional[QtGui.QPixmap] = None
        self._split_ratio: float = 0.5  # 0 = fully original, 1 = fully processed
        self._zoom = 1.0
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:  # type: ignore[override]
        ...
        base = self._original  # assume both images share geometry
        ...
        painter.drawPixmap(base_rect.topLeft(), self._processed)
        painter.restore()

        # Right side: original
        painter.save()
        painter.setClipRect(QtCore.QRectF(split_x, base_rect.top(), base_rect.right() - split_x, base_rect.height()))
        painter.drawPixmap(base_rect.topLeft(), self._original)
        painter.restore()

        # Divider line
        painter.setPen(QtGui.QPen(QtGui.QColor("#ff8800"), 2))
        painter.drawLine(split_x, base_rect.top(), split_x, base_rect.bottom())
    ...
    def _map_to_image_space(self, pos: QtCore.QPoint) -> QtCore.QPointF:
        center = QtCore.QPointF(self.width() / 2, self.height() / 2)
    ...
    def mousePressEvent(self, event: QtGui.QMouseEvent) -> None:  # type: ignore[override]
        ...
        img_pos = self._map_to_image_space(event.pos())
        base = self._original or self._processed
        if not base:
            return
        w, h = base.width(), base.height()
        base_rect = QtCore.QRectF(-w / 2, -h / 2, w, h)
        split_x = base_rect.left() + base_rect.width() * self._split_ratio
        # If click is near the split line, start split dragging, else pan
...
class SettingsPanel(QtWidgets.QScrollArea):
    ...
    def _build_thresholds_group(self) -> QtWidgets.QGroupBox:
        ...
        layout.addWidget(self._labeled("Empty-page threshold (‰, CLI default 2)", threshold_empty))
    ...
    def to_config(self) -> dict[str, str]:
        ...
        for name, widget in self._widgets.items():
            ...
            elif isinstance(widget, QtWidgets.QLineEdit):  # fallback
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        # Settings menu ("button next to File") -------------------------
    ...
    def _on_page_selected(self) -> None:
        ...
        # Show global palette info if applicable
